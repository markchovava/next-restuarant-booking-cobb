...
import { _scheduleDeleteAction } from "@/_api/actions/ScheduleActions";
...
export default function BookingLocationPage({dbData}: {dbData: any}) {
    const { 
        dataList, 
        meta, 
        links, 
    ...
    async function handleDelete(scheduleId: string | number, date: string, time: string){
        ///setIsLoading(true)
        try{
            const res = await _scheduleDeleteAction(scheduleId, date, time) 
        ...
        <section className="hidden md:block w-full overflow-x-auto rounded-lg border border-gray-400">
          <div className='min-w-[700px]'>
            ...
            <section className="bg-gray-300 font-bold text-sm lg:text-base flex items-center">
              <div className="w-[25%] border-r border-gray-400 px-2 py-2">LOCATION</div>
              <div className="w-[30%] border-r border-gray-400 px-2 py-2">DATE & TIME</div>
              <div className="w-[15%] border-r border-gray-400 px-2 py-2">TABLES TAKEN</div>
            ...
            <section key={key} className={`${i.userId && 'text-green-800 font-medium'} border-b border-gray-400 flex items-center hover:bg-gray-50 transition-colors`}>
              ...
              <div className="w-[10%] px-2 py-2 flex items-center justify-center gap-3">
                <button className="cursor-pointer group">
                  <Link href={`/admin/booking/location/${i.id}`}>
                ...
                <button 
                  onClick={() => handleDelete(i.id, i.date, i.time)}
          ...
          {dataList.map((i, key) => (
              <div key={key} className={`${i.userId && 'text-green-800 font-medium'} 
                ...
                <div className="space-y-2">
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    <p className="text-xs text-gray-500 font-medium mb-1">TABLES TAKEN</p>
