...
import { _locationUpdateAction } from '@/_api/actions/LocationActions';
...
export default function LocationEditModal({id}: {id: string | number}) {
    const { setToggleModal, 
        ...
        async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
            ...
            console.log('formData', formData)
                try {
                    const res = await _locationUpdateAction(id, formData);
                    console.log('res', res)
                    switch(res.status){
                        case 1:
                            await getData(id);
